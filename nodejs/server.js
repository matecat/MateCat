...
const corsAllow = (req, res) => {
  return allowedOrigins.some((element) => {
    if (
      element === '*' ||
      (req.headers['origin'] && req.headers['origin'] === element)
    ) {
...
const checkCandidate = (type, response, message) => {
  ...
  switch (type) {
    ...
    case COMMENTS_TYPE:
      candidate =
        response._matecatJobId === message.data.id_job &&
...
const stompMessageReceived = (body) => {
  ...
  if (!dest) {
    ...
    logger.debug([
      'Skip message, no available recipient found ',
...
const startStompConnection = () => {
  ...
  stompit.connect(connectOptions, (error, client) => {
    if (typeof client === 'undefined') {
      ...
      logger.error(
        '** client error, restarting connection in 10 seconds',
        error,
      )
      return
    }
