...
class ProjectDaoTest extends AbstractTest
{
    ...
    {
        $dao = new ProjectDao(\Model\DataAccess\Database::obtain());
        $project = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findById(self::PROJECT_ID_1);
        ...
        $this->assertSame('Project Alpha Updated', (new ProjectDao(\Model\DataAccess\Database::obtain()))->findById(self::PROJECT_ID_1)?->name);
    }
    ...
    public function changePasswordUpdatesProjectPassword(): void
    {
        ...
        $reloaded = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByIdAndPassword(self::PROJECT_ID_1, 'ppass-1-new');
    ...
    public function changeNameUpdatesProjectName(): void
    {
        ...
        $this->assertSame('Project Alpha Renamed', (new ProjectDao(\Model\DataAccess\Database::obtain()))->findById(self::PROJECT_ID_1)?->name);
    }

    #[Test]
    public function findByIdReturnsExistingAndNullForMissing(): void
    {
        $this->assertInstanceOf(ProjectStruct::class, (new ProjectDao(\Model\DataAccess\Database::obtain()))->findById(self::PROJECT_ID_1));
        $this->assertNull((new ProjectDao(\Model\DataAccess\Database::obtain()))->findById(88889999));
    }

    #[Test]
    public function findByIdAndPasswordReturnsProjectForExistingPair(): void
    {
        $project = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByIdAndPassword(self::PROJECT_ID_1, 'ppass-1');
    ...
    public function findByIdAndPasswordThrowsNotFoundForMissingPair(): void
    {
        ...
        (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByIdAndPassword(self::PROJECT_ID_1, 'wrong-password');
    }

    #[Test]
    public function existsReturnsTrueForExistingAndFalseForMissing(): void
    {
        $this->assertTrue((new ProjectDao(\Model\DataAccess\Database::obtain()))->exists(self::PROJECT_ID_1));
        $this->assertFalse((new ProjectDao(\Model\DataAccess\Database::obtain()))->exists(88889999));
    }

    #[Test]
    public function findByTeamIdSupportsFiltersLimitAndOffset(): void
    {
        $all = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByTeamId(self::TEST_TEAM_ID);
        $this->assertCount(3, $all);

        $byId = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByTeamId(self::TEST_TEAM_ID, ['search' => ['id' => self::PROJECT_ID_1]]);
        $this->assertCount(1, $byId);
        $this->assertSame(self::PROJECT_ID_1, $byId[0]->id);

        $byName = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByTeamId(self::TEST_TEAM_ID, ['search' => ['name' => 'Project Beta']]);
        $this->assertCount(1, $byName);
        $this->assertSame('Project Beta', $byName[0]->name);

        $paged = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByTeamId(self::TEST_TEAM_ID, ['limit' => 1, 'offset' => 1]);
    ...
    {
        $this->assertSame(3, (new ProjectDao(\Model\DataAccess\Database::obtain()))->getTotalCountByTeamId(self::TEST_TEAM_ID));
        $this->assertSame(1, (new ProjectDao(\Model\DataAccess\Database::obtain()))->getTotalCountByTeamId(self::TEST_TEAM_ID, ['search' => ['id' => self::PROJECT_ID_2]]));
        $this->assertSame(1, (new ProjectDao(\Model\DataAccess\Database::obtain()))->getTotalCountByTeamId(self::TEST_TEAM_ID, ['search' => ['name' => 'Project Alpha']]));
    }

    #[Test]
    public function findByJobIdReturnsProjectOrNull(): void
    {
        $project = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByJobId(self::JOB_ID_1);

        $this->assertInstanceOf(ProjectStruct::class, $project);
        $this->assertSame(self::PROJECT_ID_1, $project?->id);
        $this->assertNull((new ProjectDao(\Model\DataAccess\Database::obtain()))->findByJobId(88889999));
    }

    #[Test]
    public function findByIdCustomerReturnsProjectsForCustomer(): void
    {
        $projects = (new ProjectDao(\Model\DataAccess\Database::obtain()))->findByIdCustomer('customer-a');
    ...
    {
        $rows = (new ProjectDao(\Model\DataAccess\Database::obtain()))->getProjectAndJobData(self::PROJECT_ID_1);
    ...
    {
        $ok = (new ProjectDao(\Model\DataAccess\Database::obtain()))->updateAnalysisStatus(self::PROJECT_ID_1, 'DONE', 123);
    ...
    {
        $affected = (new ProjectDao(\Model\DataAccess\Database::obtain()))->changeProjectStatus(self::PROJECT_ID_1, 'BUSY');

        $this->assertSame(1, $affected);
        $this->assertSame('BUSY', (new ProjectDao(\Model\DataAccess\Database::obtain()))->findById(self::PROJECT_ID_1)?->status_analysis);
    }
    ...
    public function cacheDestroyMethodsAreCallableAndReturnBool(): void
    {
        ...
        $this->assertIsBool((new ProjectDao(\Model\DataAccess\Database::obtain()))->destroyCacheByIdAndPassword(self::PROJECT_ID_1, 'ppass-1'));
    ...
    {
        $this->assertFalse((new ProjectDao(\Model\DataAccess\Database::obtain()))->isGDriveProject(self::PROJECT_ID_1));
    }
