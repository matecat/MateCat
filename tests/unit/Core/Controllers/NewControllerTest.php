...
class NewControllerTest extends AbstractTest
{
    ...
    public function testValidateTheRequestWithNewPrivateTmKey(): void
    {
        ...
        $this->assertIsArray($validateParameters);
        $this->assertArrayHasKey('tm_keys', $validateParameters);
        
        $tmKeys = json_decode($validateParameters['tm_keys'], true);
        $this->assertCount(1, $tmKeys);
        $this->assertEquals('New resource created for project {{pid}}', $tmKeys[0]['name']);
        $this->assertEquals('rw', $tmKeys[0]['r']);
        $this->assertEquals('rw', $tmKeys[0]['w']);
    }
