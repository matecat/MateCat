...
function resolveCommentThread( $commentData ) {
    $db = Database::obtain();

    $id_job        = (int) $commentData['id_job'];
    $id_segment    = (int) $commentData['id_segment'];
    $resolve_date  = gmdate('Y-m-d H:i:s');

    $db->begin();
    $insert = insertCommentRecord($commentData);
    ...
    try {
        ...
        $update = "UPDATE comments SET resolve_date = '$resolve_date' " .
            " WHERE id_segment = $id_segment " .
            " AND id_job = $id_job " .
    ...
    return md5($id_job . '-' . $id_segment . '-' . $resolve_date);
}

function insertCommentRecord( $commentData ) {
    $db    = Database::obtain();
