...
class SegmentTranslationDao extends AbstractDao
{
     ...
     */
    public function getSegmentTranslationsModifiedByRevisorWithIssueCount(
        int $id_job,
        string $password,
        int $source_page
    ): array {
     ...
     * @throws ReflectionException
     */
    public function getAllSegmentsByIdListAndJobId(array $id_list, int $jobId, int $ttl = 0): array
    {
        ...
        foreach ($chunked_id_list as $list) {
             ...
             */
            $result = $this->setCacheTTL($ttl)->_fetchObjectMap(
                $stmt,
     ...
     * @throws Exception
     */
    public function updateTranslationAndStatusAndDateByList(array $translation_struct): int
    {
     ...
     * @throws ReflectionException
     */
    public function findBySegmentAndJob(int $id_segment, int $id_job, int $ttl = 0): ?SegmentTranslationStruct
    {
        $conn = $this->database->getConnection();
         ...
         * @var SegmentTranslationStruct[] $result
         */
        $result = $this->setCacheTTL($ttl)->_fetchObjectMap($stmt, SegmentTranslationStruct::class, [
            'id_job' => $id_job,
     ...
     * @throws PDOException
     */
    public function updateLastTranslationDateByIdList(array $segmentIdList, string $date): void
    {
     ...
     * @throws Exception
     */
    public function getUnchangeableStatus(JobStruct $chunk, array $segments_ids, string $status, ?int $source_page): array
    {
        $where_values = [];
        $conn = $this->database->getConnection();
     ...
     * @throws Exception
     */
    public function updateTranslationAndStatusAndDate(SegmentTranslationStruct $translation_struct): int
    {
        $values = [
            'fields' => [
                'translation',
        ...
        return $this->updateStruct($translation_struct, $values);
    }
     ...
     * @throws PDOException
     */
    public function getMaxSegmentIdsFromJob(JobStruct $jStruct): array
    {
        $conn = $this->database->getConnection();
     ...
     * @throws PDOException
     */
    public function updateFirstTimeOpenedContribution(array $data, array $where): void
    {
        $this->updateFields($data, $where);
    }
    ...
    public function propagateTranslation(
        ...
        try {
            $stmt = $this->database->getConnection()->prepare($queryTotals);
            ...
            throw new Exception(
                "Error in counting total words for propagation: " . $e->getCode() . ": " . $e->getMessage()
     ...
     */
    public function getLast10TranslatedSegmentIDsInLastHour(int $id_job): ?array
    {
        $now = new DateTime();
     ...
     * @throws PDOException
     */
    public function getWordsPerSecond(int $id_job, array $estimation_seg_ids = []): array
    {
        $query = "
            SELECT
                   Round(
     ...
     * @throws PDOException
     */
    public function updateSuggestionsArray(int $id_segment, array $suggestions): void
    {
