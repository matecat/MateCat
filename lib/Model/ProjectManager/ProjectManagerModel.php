...
class ProjectManagerModel {
    ...
    public static function insertFile( ArrayObject $projectStructure, string $file_name, string $mime_type, string $fileDateSha1Path, ArrayObject $meta ): string {
        ...
        try {
            ...
            LoggerFactory::doJsonLog( "Database insert error: {$e->getMessage()} " );
    ...
    public static function insertPreTranslations( &$query_translations_values ) {
        ...
        LoggerFactory::doJsonLog( "Pre-Translations: Total Rows to insert: " . count( $query_translations_values ) );
        ...
        LoggerFactory::doJsonLog( "Pre-Translations: Total Queries to execute: " . count( $query_translations_values ) );
        ...
        foreach ( $query_translations_values as $i => $chunk ) {
            ...
            try {
                ...
                LoggerFactory::doJsonLog( "Pre-Translations: Executed Query " . ( $i + 1 ) );
            } catch ( PDOException $e ) {
    ...
    public static function bulkInsertSegmentNotes( $notes ) {
        ...
        LoggerFactory::doJsonLog( "Notes: Total Rows to insert: " . count( $chunked ) );
        ...
        try {
            ...
            foreach ( $chunked as $i => $chunk ) {
                ...
                LoggerFactory::doJsonLog( "Notes: Executed Query " . ( $i + 1 ) );
            }

        } catch ( Exception $e ) {
            LoggerFactory::doJsonLog( "Notes import - DB Error: " . $e->getMessage() );
            LoggerFactory::doJsonLog( "Notes import - Statement: " . $stmt->queryString );
            LoggerFactory::doJsonLog( "Notes Chunk Dump: " . var_export( $chunk, true ) );
            LoggerFactory::doJsonLog( "Notes Flattened Values Dump: " . var_export( $flattened_values, true ) );
    ...
    public static function bulkInsertSegmentMetaDataFromAttributes( $notes ) {
        ...
        $chunked = array_chunk( $insert_values, $chunk_size );
        LoggerFactory::doJsonLog( "Notes attributes: Total Rows to insert: " . count( $chunked ) );
        ...
        try {
            ...
            foreach ( $chunked as $i => $chunk ) {
                ...
                $flattened_values = array_reduce( $chunk, 'array_merge', [] );
                $stmt->execute( $flattened_values );
                LoggerFactory::doJsonLog( "Notes attributes: Executed Query " . ( $i + 1 ) );
            }

        } catch ( Exception $e ) {
            LoggerFactory::doJsonLog( "Notes attributes import - DB Error: " . $e->getMessage() );
            LoggerFactory::doJsonLog( "Notes attributes import - Statement: " . $stmt->queryString );
            LoggerFactory::doJsonLog( "Notes attributes Chunk Dump: " . var_export( $chunk, true ) );
            LoggerFactory::doJsonLog( "Notes attributes Flattened Values Dump: " . var_export( $flattened_values, true ) );
    ...
    public static function bulkInsertContextsGroups( $projectStructure ) {
        ...
        try {
            ...
            LoggerFactory::doJsonLog( "Trans-Unit Context Groups import - DB Error: " . $e->getMessage() );
            LoggerFactory::doJsonLog( "Trans-Unit Context Groups import - Statement: " . $stmt->queryString );
            LoggerFactory::doJsonLog( "Trans-Unit Context Groups Chunk Dump: " . var_export( $chunk, true ) );
            LoggerFactory::doJsonLog( "Trans-Unit Context Groups Flattened Values Dump: " . var_export( $flattened_values, true ) );
            throw new Exception( "Notes import - DB Error: " . $e->getMessage(), 0, $e );
        }
