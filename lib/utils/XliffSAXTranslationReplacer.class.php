...
class XliffSAXTranslationReplacer {
	...
	public function __construct( $inputFilename, $segments, $trg_lang = null, $outputPath = false ) {

		//check if file is readable
		if ( !( $this->originalFP = fopen( $inputFilename, "r" ) ) ) {
			die( "could not open XML input" );
		}

		//default for output file
		if(!$outputPath){
			$outputPath=$inputFilename.'out.xlf';
		}

		//open output file
		$this->outputFP    = fopen( $outputPath, 'w+' );
	...
	public function __destruct() {
		if(is_resource($this->originalFP)) fclose( $this->originalFP);
		if(is_resource($this->outputFP)) fclose( $this->outputFP );
	}
