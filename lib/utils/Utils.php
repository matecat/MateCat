...
class Utils {
     ...
     * @param $grants string    One of the following strings : "r", "w", "rw"
     * @return array|mixed
     * @throws Exception Throws Exception on query fail or if grants string is wrong
     */
    public static function getJobTmKeys( $job_id, $job_pass, $grants ) {
        $accepted_grants = array( "r", "w", "rw" );

        if ( !in_array( $grants, $accepted_grants ) ) {
            throw new Exception ( __METHOD__ . " -> Invalid grant string." );
        }

        $tmKeys = getJobTmKeys( $job_id, $job_pass );
        ...
        switch ( $grants ) {
            case 'r' :
                $tmKeys = array_filter( $tmKeys, array( "Utils", 'filterTmKeysByReadGrant' ) );
                break;
            case 'w' :
     ...
     * @return mixed
     */
    public static function setJobTmKeys( $job_id, $job_password, $tmKeys ) {
        return setJobTmKeys( $job_id, $job_password, json_encode( $tmKeys ) );
    }
