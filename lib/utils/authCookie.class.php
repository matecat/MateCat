...
class AuthCookie {
	...
	public static function setCredentials($username){
		$new_expire_date=time()+INIT::$AUTHCOOKIEDURATION;
		$new_cookie_data=array(
				'username'=>$username,
				'expire_date'=>$new_expire_date,
				'hash'=>hash('sha256',INIT::$AUTHSECRET.$username.$new_expire_date)
				);
	...
	private static function validate(){
		...
		if(is_array($cookie)){
			...
			$expected_hash=hash('sha256',INIT::$AUTHSECRET.$cookie['username'].$cookie['expire_date']);
			...
			if($cookie['hash']==$expected_hash and time()<$cookie['expire_date']){
				...
				self::setCredentials($cookie['username']);
				//confirm login
				$valid=true;
			}else{
