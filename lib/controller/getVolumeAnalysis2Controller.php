...
class getVolumeAnalysis2Controller extends ajaxcontroller {
    ...
    public function __construct() {
        ...
        //$rand_pid = array(1000, 1522, 1562, 1041, 862, 1564, 1302, 1483, 1608, 1602);
        //$pid_index = mt_rand(0, count($rand_pid) - 1);
        //$this->id_project = $rand_pid[$pid_index];
    }
    ...
    public function doAction() {
        ...
        foreach ($res as $r) {
            ...
            if (!array_key_exists($r['id_file'], $return_data[$jid]['file_details'])) {
                //$return_data[$jid]['file_details'][$r['id_file']]=$total_init;
                $return_data[$jid]['file_details']["1703"] = $total_init;
                $return_data[$jid]['file_details']["1704"] = $total_init;
            }

            if ($r['suggestion_source'] == "MT") {
                //$w=$return_data[$jid]['file_details'][$r['id_file']]["MT"] + $words;
                //$words_print = number_format($w, 0, ".", ",");
                //$return_data[$jid]['file_details'][$r['id_file']]["MT"]+=array($words,$words_print);
            ...
            if ($r['suggestion_match'] == 100) {
                //$w=$return_data[$jid]['file_details'][$r['id_file']]["TM_100"] + $words;
                //$words_print = number_format($w, 0, ".", ",");
                //$return_data[$jid]['file_details'][$r['id_file']]["TM_100"]+=array($words,$words_print);
