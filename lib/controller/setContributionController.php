...
class setContributionController extends ajaxcontroller {
	...
	public function doAction() {
		...
		if (is_null($this->source) || $this->source === '') {
			...
			Log::doLog( "missing source segment");

			Log::doLog ($_POST);
			Utils::sendErrMailReport("missing source segment");

		}

		if ( is_null($this->target) || $this->target === '' ) {
			$this->result['error'][] = array("code" => -2, "message" => "missing target segment");
			Log::doLog( "missing target segment");
			Log::doLog ($_POST);
			Utils::sendErrMailReport("missing target segment");
		...
		if (empty($this->source_lang)) {
			...
			Log::doLog( "missing source lang");
			Log::doLog ($_POST);
			Utils::sendErrMailReport("missing source lang");

		}

		if (empty($this->target_lang)) {
			$this->result['error'][] = array("code" => -2, "message" => "missing target lang");
			Log::doLog( "missing targt lang");
			Log::doLog ($_POST);
			Utils::sendErrMailReport("missing target lang");
