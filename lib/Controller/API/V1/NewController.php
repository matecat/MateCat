...
class NewController extends KleinController
{
    ...
    public function create()
    {
        try {
            ...
            foreach ( $arFiles as $file_name ) {
                ...
                if ( $ext == "zip" ) {
                    ...
                    if ( $conversionHandler->uploadError ) {
                        ...
                        if ( $errors !== null and !empty( $errors ) ) {
            ...
            if ( isset( $result[ 'data' ] ) && !empty( $result[ 'data' ] ) ) {
                foreach ( $result[ 'data' ] as $zipFileName => $zipFiles ) {
            ...
            $projectStructure[ 'due_date' ]                     = ( !isset( $this->postInput[ 'due_date' ] ) ? null : Utils::mysqlTimestamp( $request[ 'due_date' ] ) );
            $projectStructure[ 'target_language_mt_engine_id' ] = $request[ 'target_language_mt_engine_id' ];
            ...
            // mmtGlossaries
            if ( $request['mmtGlossaries'] ) {
                $projectStructure[ 'mmt_glossaries' ] = $request['mmtGlossaries'];
            }

            // DeepL
            if ( $request['mt_engine'] instanceof Engines_DeepL and $request['deepl_formality'] !== null ) {
                $projectStructure[ 'deepl_formality' ] = $request['deepl_formality'];
            }

            if ( $request['mt_engine'] instanceof Engines_DeepL and $request['deepl_id_glossary'] !== null ) {
    ...
    private function validateTheRequest(): array
    {
        ...
        [$private_tm_user, $private_tm_pass, $private_tm_key, $new_keys] = $this->validateTmAndKeys($private_tm_key);
        ...
        $project_features = $this->appendFeaturesToProject($project_completion);
        $target_language_mt_engine_id = $this->generateTargetEngineAssociation($target_lang, $mt_engine, $target_language_mt_engine_id);
    ...
    private function validateMetadataParam($metadata = null)
    {
        if ( !empty( $metadata ) ) {
            ...
            $parsedMetadata                = json_decode( $metadata, $assoc, $depth );
        ...
        // @TODO still is use???
        $metadata = $this->featureSet->filter( 'filterProjectMetadata', $metadata, $_POST );
        $metadata = $this->featureSet->filter( 'createProjectAssignInputMetadata', $metadata, [
            'input' => $_POST
        ] );
     ...
     * @throws Exception
     */
    private function validateEngines($tms_engine, $mt_engine): array
    {
        if ( !isset( $tms_engine ) ) {
            $tms_engine = 1;
        }
        ...
        if ( $tms_engine != 0 ) {
            Engine::getInstance( $tms_engine );
        }

        if ( $mt_engine != 0 and $mt_engine != 1 ) {
            if ( !$this->userIsLogged ) {
                throw new InvalidArgumentException( "Invalid MT Engine.", -2 );
            } else {
                $testEngine = Engine::getInstance( $mt_engine );
                if ( $testEngine->getEngineRow()->uid != $this->getUser()->uid ) {
                    throw new InvalidArgumentException( "Invalid MT Engine.", -21 );
                }
            }
        }
     ...
     * @throws Exception
     */
    private function appendFeaturesToProject($project_completion = null): array
    {
        $projectFeatures = [];
     ...
     * @throws Exception
     */
    protected function validateTmAndKeys($private_tm_key): array
    {
        $new_keys = [];
        ...
        try {
            $private_tm_key = $this->parseTmKeyInput($private_tm_key);
        } catch ( Exception $e ) {
            throw new InvalidArgumentException( $e->getMessage(), -6 );
        }

        if ( count($private_tm_key) > self::MAX_NUM_KEYS ) {
            throw new InvalidArgumentException( "Too much keys provided. Max number of keys is " . self::MAX_NUM_KEYS, -2 );
        }
     ...
     * @return \LQA\ModelStruct|null
     * @throws Exception
     */
    private function validateQaModel($id_qa_model = null): ?\LQA\ModelStruct
    {
     ...
     * @return \Filters\FiltersConfigTemplateStruct|mixed|null
     * @throws Exception
     */
