...
if ($language !~ /^(cs|en|fr|it)$/){
	print STDERR "No built-in rules for language $language, back off to en for default behaviour.\n";
	$language = "en";
}
...
sub detokenize {
	...
	$text = " $text ";
        ...
        # manage Italian word "po'"
        if ($language eq "it") { $text =~ s/\bpo'/POREPLACEWORD/g; }
	...
	$text .= "\n" unless $text =~ /\n$/;
        ...
        # manage Italian word "po'"
        if ($language eq "it") { $text =~ s/POREPLACEWORD/po'/g; }
