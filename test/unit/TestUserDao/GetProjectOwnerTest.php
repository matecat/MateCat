...
class GetProjectOwnerTest extends AbstractTest
{
    ...
    public function setUp()
    {
        ...
        $this->sql_insert_user = "INSERT INTO ".INIT::$DB_DATABASE. ".`users` (`uid`, `email`, `salt`, `pass`, `create_date`, `first_name`, `last_name`, `api_key` ) VALUES (NULL, '".$this->email_owner."', '12345trewq', '987654321qwerty', '2016-04-11 13:41:54', 'Bar', 'Foo', '');";
        $this->database_instance->query($this->sql_insert_user);
        $this->uid_user=$this->database_instance->getConnection()->lastInsertId();

        $this->sql_delete_user = "DELETE FROM users WHERE uid='" . $this->uid_user . "';";
        ...
        $this->job_struct = new Jobs_JobStruct(
            array('id' => null, //SET NULL FOR AUTOINCREMENT
