...
describe('word-count box flash animation', () => {
  beforeEach(() => {
    jest.useFakeTimers()
  })
  ...
  test('flashes and then clears when the saving percentage changes', () => {
    const {rerender} = render(
      <AnalyzeHeader data={buildData()} project={project} />,
    )

    rerender(
      <AnalyzeHeader
        data={buildData({total_equivalent: 200})}
        project={project}
      />,
    )
  ...
  test('does not flash when the saving percentage stays the same', () => {
    ...
    rerender(<AnalyzeHeader data={buildData()} project={project} />)
