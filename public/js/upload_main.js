...
window.UI = {
  init: function () {
    ...
    config.maxFileSizePrint =
      parseInt(Math.pow(1024, base - Math.floor(base)) + 0.5) + ' MB'
  ...
  restartConversions: function () {
    ...
    $('.template-download, .template-upload').each(function () {
      if (config.conversionEnabled) {
        ...
        var filesize = $('.size span', filerow).length
          ? parseFloat($('.size span', filerow).text().split(' ')[0]) * 1000000
          : 0
  ...
  checkFailedConversionsNumber: function () {
    var n = $(
      '.template-download.failed, .template-upload.failed, .template-download.has-errors, .template-upload.has-errors',
    ).length
    if (n > 1) {
  ...
  addEvents: function () {
    ...
    $('#fileupload').fileupload('option', {
      ...
      fileInput: $(
        '#fileupload .multiple-button, .btncontinue .multiple-button',
      ),
    ...
    $('#fileupload')
      ...
      .bind('fileuploadsend', function (e, data) {
        ...
        data.context &&
          $('.progress', $(data.context[0])).before(
            '<div class="operation">Uploading</div>',
          )
      })
      ...
      .bind('fileuploadadded fileuploaddestroyed', function (e, data) {
        if ($('.upload-table tr').length) {
          $('.upload-files').addClass('uploaded')
          if (APP.hideGDLink) APP.hideGDLink()
        } else {
          $('.upload-files').removeClass('uploaded')
          if (APP.showGDLink) APP.showGDLink()
        }
      })
      ...
      .bind('fileuploadcompleted', function (e, data) {
        ...
        if (data.result[0]) {
          ...
          fileSpecs = {
            error:
              typeof data.result.errors !== 'undefined'
    ...
    $('#clear-all-files').bind('click', function (e) {
      ...
      CreateProjectActions.hideErrors()
      $(
        '.template-download .delete button, .template-upload .cancel button',
      ).click()
    })
    ...
    $('#delete-failed-conversions').bind('click', function (e) {
      e.preventDefault()
      $(
        '.template-download.failed .delete button, .template-download.has-errors .delete button, .template-upload.failed .cancel button, .template-upload.has-errors .cancel button',
      ).click()
    })
...
var convertFile = function (fname, filerow, filesize, enforceConversion) {
  ...
  convertFileRequest({
    ...
    .then(function ({data, errors}) {
      ...
      if (data.code == 1 || data.code == 2) {
        ...
        if (
          ...
          typeof data.data['zipFiles'] !== 'undefined'
        ) {
          ...
          $.each(zipFiles, function (i, file) {
            ...
            $(rowClone)
              ...
              .html(
                "<i class='icon-make-group icon'/>" +
                  '<span class="zip_internal_file">' +
                  file['name'].replace(/&/g, '&amp;') +
                  '</span>',
              )
            ...
            $(rowClone)
              .find('.size')
              .first()
              .html(UI.getPrintableFileSize(file['size']))
            ...
            var newDataUrl = oldDataUrl.replace(
              /file=[^&]+/g,
          ...
          $('.name').each(function () {
            ...
            if (
              $(this).text().split('.')[currSplitLength] == 'tmx' ||
    ...
    .catch(function (d) {
      ...
      $('td.size', filerow)
        ...
        .attr('colspan', '2')
        .append(
          '<span class="label label-important">Error: </span>Server error, try again.',
        )
  ...
  $('.size', filerow)
    .next()
    .append(
      '<div class="operation">Importing</div><div class="converting progress progress-success progress-striped active ui-progressbar ui-widget ui-widget-content ui-corner-all" aria-valuenow="0" aria-valuemax="100" aria-valuemin="0" role="progressbar"><div class="ui-progressbar-value ui-widget-header ui-corner-left" style="width: 0%;"></div></div>',
    )
...
var checkAnalyzability = function (who) {
  if (
    $('.upload-table tr:not(.failed)').length ||
    $('.gdrive-upload-table tr:not(.failed)').length
  ) {
    var res = true
    $(
      '.upload-table tr:not(.failed), .gdrive-upload-table tr:not(.failed)',
    ).each(function () {
    ...
    if (
      !$(
        '.upload-table tr:not(.failed, .tmx), .gdrive-upload-table tr:not(.failed)',
      ).length
    ) {
      return false
    }
