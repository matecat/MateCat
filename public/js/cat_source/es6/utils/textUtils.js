...
const TEXT_UTILS = {
  ...
  getCharsValue: (char) => new Blob([char]).size,
  getCJKCharsValue: (char) => TEXT_UTILS.getCharsValue(char),
  getEmojiCharsValue: (char) => TEXT_UTILS.getCharsValue(char),
  getCJKMatches: (value) => {
    const regex =
    ...
    while ((match = regex.exec(value)) !== null) {
      ...
      result.push({
        match: char,
        index: match.index,
        counterValue: TEXT_UTILS.getCJKCharsValue(char),
      })
    }
  ...
  getEmojiMatches: (value) => {
    ...
    while ((match = regex.exec(value)) !== null) {
      ...
      result.push({
        match: match[0],
        index: match.index,
        counterValue: TEXT_UTILS.getEmojiCharsValue(char),
      })
    }
