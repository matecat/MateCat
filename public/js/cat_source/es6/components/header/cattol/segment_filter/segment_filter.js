...
let SegmentFilterUtils = {
    ...
    tryToFocusLastSegment : () => {
        var segment = SegmentStore.getSegmentByIdToJS( SegmentFilterUtils.getStoredState().lastSegmentId ) ;
    ...
    setStoredState : function( data ) {
        SegmentFilterUtils.cachedStoredState = $.extend( SegmentFilterUtils.getStoredState(), data );
        localStorage.setItem(SegmentFilterUtils.keyForLocalStorage(), JSON.stringify( SegmentFilterUtils.cachedStoredState ) ) ;
    },
    ...
    filterSubmit : function( params ,extendendLocalStorageValues) {
        ...
        return $.getJSON(path).pipe(function( data ) {
            ...
            } else if ( lastSegmentId && !SegmentFilterUtils.segmentIsInSample( lastSegmentId, data[ 'segment_ids' ] ) ) {
