...
class SegmentCommentsContainer extends React.Component {
    ...
    getComments() {
        ...
        if ( this.state.comments.length > 0 ) {
            let thread_wrap = [], thread_id = 0, count = 0, commentsHtml = [], threadClass;
            ...
            comments.forEach( ( comment, i ) => {
                ...
                if ( comment.thread_id !== thread_id ) {
                    ...
                    if ( thread_wrap.length > 0 ) {
                        commentsHtml.push( <div key={'thread-' + i} className={"mbc-thread-wrap mbc-clearfix " + threadClass} data-count={count}>
                            {thread_wrap}</div> );
                        count = 0;
                    }
                    thread_wrap = [];
                ...
                if ( Number( comment.message_type ) === this.types.resolve ) {
                    thread_wrap.push( <div className="mbc-resolved-comment" key={'comment-' + i}>
                            <span className="mbc-comment-resolved-label">
                            <span className="mbc-comment-username mbc-comment-resolvedby">{comment.full_name}</span>
                    ...
                    thread_wrap.push( <div className="mbc-show-comment mbc-clearfix" key={'comment-' + i}>
                        <span className="mbc-comment-label mbc-comment-username mbc-comment-username-label mbc-truncate">{comment.full_name}</span>
                        <span className="mbc-comment-label mbc-comment-email-label mbc-truncate">{comment.email}</span>
                        <div className="mbc-comment-info-wrap mbc-clearfix">
                            <span className="mbc-comment-info mbc-comment-time pull-left">{comment.formatted_date}</span>
                        </div>
                        <p className="mbc-comment-body" dangerouslySetInnerHTML={ { __html: text } }/>
                    </div> )
                }
            ...
            if (thread_wrap.length > 0) {
                commentsHtml.push( <div key={'thread-' + 900} className={"mbc-thread-wrap mbc-clearfix " + threadClass} data-count={count}>
                    {thread_wrap}{resolveButton}</div> );
            }
