...
class SegmentPlaceholderLite extends React.Component {
    constructor(props) {
        ...
        const cleanSource = SegmentUtils.checkCurrentSegmentTPEnabled(this.props.segment) ?
            DraftMatecatUtils.cleanSegmentString(source) : source;
        const contentEncodedSource = DraftMatecatUtils.encodeContent(plainEditorStateSource, cleanSource);
        const {editorState: editorStateSource, tagRange: tagRangeSource} =  contentEncodedSource;

        // --- Prepare Translation
        const plainEditorStateTarget = EditorState.createEmpty(decorator);
        const translation =  DraftMatecatUtils.unescapeHTMLLeaveTags(this.props.segment.translation);
        ...
        const contentEncodedTarget = DraftMatecatUtils.encodeContent(plainEditorStateTarget, cleanTranslation);
        const {editorState: editorStateTarget, tagRange: tagRangeTarget} =  contentEncodedTarget;
        // --- Set Editor content
        this.state = {
            editorStateSource,
            editorStateTarget
    ...
    componentDidMount() {
        ...
        this.containerRef.style.cssText = `width:${window.innerWidth - 10}px !important;`;
...
class TagEntity extends Component {
    ...
    render() {
        ...
        return <span className={`tag ${tagStyle} `}
                  unselectable="on"
