...
class SegmentFooterTabGlossary extends React.Component {
    ...
    copyItemInEditArea(translation) {
        GlossaryUtils.copyGlossaryItemInEditarea(
            TagUtils.decodePlaceholdersToText(translation, true),
            this.props.segment
        );
    }
    onPasteEvent(e) {
        ...
        if (_.size(this.props.segment.glossary)) {
            ...
            $.each(this.props.segment.glossary, function (name, value) {
                $.each(value, function (index, match) {
                    ...
                    let html = (
                        <div key={name + '-' + index} ref={(match) => (self.matches[name] = match)}>
                            ...
                            <ul className="graysmall" data-id={match.id}>
                                <li className="sugg-source">
                                    ...
                                    <span
                                        ...
                                        dangerouslySetInnerHTML={self.allowHTML(
                                            TagUtils.decodePlaceholdersToText(leftTxt, true)
                                        )}
                                    />
                                </li>
                                <li className="b sugg-target" onMouseDown={() => self.copyItemInEditArea(rightTxt)}>
                                    <span
                                        ...
                                        dangerouslySetInnerHTML={self.allowHTML(
                                            TagUtils.decodePlaceholdersToText(rightTxt, true)
                                        )}
                                        onKeyPress={self.updateGlossaryItem.bind(self, name)}
                                    />
                                </li>
                                <li className="details">
                                    {!match.comment || match.comment === '' ? (
                                        ...
                                        <div
                                            ...
                                            dangerouslySetInnerHTML={self.allowHTML(
                                                TagUtils.decodePlaceholdersToText(commentOriginal, true)
                                            )}
                                            // onKeyPress={self.updateGlossaryItem.bind(self, name)}
                                        />
                                    )}
