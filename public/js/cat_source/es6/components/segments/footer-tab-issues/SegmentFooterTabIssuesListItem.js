...
class SegmentFooterTabIssuesListItem extends React.Component {
    ...
    addComment(e) {
        ...
        SegmentActions.submitComment(this.props.issue.id_segment, this.props.issue.id, data)
            .done(function () {
                self.setState({
                    comment_text: '',
                });
    ...
    render() {
        ...
        commentViewButtonClass =
            this.props.issue.comments.length > 0 ? commentViewButtonClass + ' re-message' : commentViewButtonClass;
        ...
        if (htmlCommentLines.length > 0) {
            renderHtmlCommentLines = <div className="re-comment-list">{htmlCommentLines}</div>;
        }
        let commentSection = (
