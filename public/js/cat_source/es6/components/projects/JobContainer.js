...
class JobContainer extends React.Component {
    ...
    changePassword(revision_number) {
        ...
        switch ( revision_number ) {
            ...
            case 1: {
                this.oldPassword = this.props.job.get('revise_passwords').get(0).get("password");
                label = "Revise";
                break;
            }
            case 2: {
                this.oldPassword = this.props.job.get('revise_passwords').get(1).get("password");
                label = "2nd Revise";
                break;
            }
        }
        this.props.changeJobPasswordFn(this.props.job.toJS(), this.oldPassword, revision_number).done(function (data) {
            let notification = {
                title: 'Change job ' + label + ' password',
                text: 'The ' + label + ' password has been changed. <a class="undo-password">Undo</a>',
                type: 'warning',
                position: 'bl',
                allowHtml: true,
                timer: 10000,
            ...
            ManageActions.changeJobPassword(self.props.project, self.props.job, data.password, data.undo, revision_number);
            setTimeout(function () {
                $('.undo-password').off('click');
                $('.undo-password').on('click', function () {
                    APP.removeNotification(boxUndo);
                    self.props.changeJobPasswordFn(self.props.job.toJS(), data.password, revision_number, 1, self.oldPassword ).done(function (data) {
                        notification = {
    ...
    removeTranslator() {
        let self = this;
        this.oldPassword = this.props.job.get('password');
        this.props.changeJobPasswordFn(this.props.job.toJS(), this.oldPassword).done(function (data) {
            let notification = {
            ...
            let boxUndo = APP.addNotification(notification);
            let translator = self.props.job.get('translator');
            ManageActions.changeJobPassword(self.props.project, self.props.job, data.password, data.undo, null);
            setTimeout(function () {
                $('.undo-password').off('click');
                $('.undo-password').on('click', function () {
                    APP.removeNotification(boxUndo);
                    self.props.changeJobPasswordFn(self.props.job.toJS(),data.password, null,  1, self.oldPassword).done(function (data) {
                        notification = {
                            title: 'Change job password',
                            text: 'The previous password has been restored.',
                            type: 'warning',
    ...
    getWarningsMenuItem() {
        ...
        if (warnings > 0) {
            ...
            let tooltipText = 'Click to see issues';
            icon = (
                <a className="ui icon basic button " href={url} target="_blank" data-position="top center">
                    <i className="icon-notice icon red" />
                    {tooltipText}
                </a>
            );
    ...
    getCommentsMenuItem() {
        ...
        if (openThreads > 0) {
            ...
            if (this.props.job.get('open_threads_count') === 1) {
                icon = (
                    <a className=" ui icon basic button " href={translatedUrl} target="_blank">
                        <i className="icon-uniE96B icon" />
    ...
    getSplitOrMergeButton(splitUrl, mergeUrl) {
        if (this.props.isChunk) {
            return (
                <a className="btn waves-effect merge waves-dark" target="_blank" href={mergeUrl}>
    ...
    getOutsourceButton() {
        ...
        if (this.props.job.get('outsource')) {
            if (this.props.job.get('outsource').get('id_vendor') == '1') {
                label = (
                    <a className="open-outsourced ui button " onClick={this.openOutsourceModal.bind(this, false, true)}>
                        View status
                    </a>
                );
            }
            ...
            if (this.props.job.get('outsource').get('id_vendor') == '1') {
                ...
                outsourceDeliveryPrice = (
                    <div className="job-price">
                        <span className="valuation">{this.props.job.get('outsource').get('currency')} </span>
    ...
    render() {
        ...
        if (
            ...
            this.props.job.get('stats').get('revises').get(1).get('advancement_wc') !== 0.0
        ) {
            ...
            approvedPerc = approved
                ? (approved.get('advancement_wc') * 100) / this.props.job.get('stats').get('TOTAL')
                : approvedPerc;
            ...
            approvedPerc2ndPass = approved2ndPass
                ? (approved2ndPass.get('advancement_wc') * 100) / this.props.job.get('stats').get('TOTAL')
                : approved2ndPass;
                        ...
                        >
                                ...
                                <div className="progr">
                                    <div className="meter">
                                        <a
                                            ...
                                            data-html={
                                                'Rejected ' +
                                                this.props.job.get('stats').get('REJECTED_PERC_FORMATTED') +
                                                '%'
                                            }
                                            style={{ width: this.props.job.get('stats').get('REJECTED_PERC') + '%' }}
                                        ...
                                        {approvedPercFormatted2ndPass ? (
                                            <a
                                                ...
                                                data-variation="tiny"
                                                data-html={'Approved ' + approvedPercFormatted2ndPass + '%'}
                                                style={{ width: approvedPerc2ndPass + '%' }}
                                                ref={(tooltip) => (this.approved2ndPassTooltip = tooltip)}
                                            />
                                        ) : null}
                                        <a
                                            ...
                                            data-html={
                                                'Translated ' +
                                                this.props.job.get('stats').get('TRANSLATED_PERC_FORMATTED') +
                                                '%'
                                            }
                                            style={{ width: this.props.job.get('stats').get('TRANSLATED_PERC') + '%' }}
                                        ...
                                        <a
                                            ...
                                            data-variation="tiny"
                                            data-html={
                                                'Draft ' + this.props.job.get('stats').get('DRAFT_PERC_FORMATTED') + '%'
                                            }
                                            style={{ width: this.props.job.get('stats').get('DRAFT_PERC') + '%' }}
                            ...
                            <div className="job-payable">
                                <a href={analysisUrl} target="_blank">
                                    <span id="words">{this.props.job.get('stats').get('TOTAL_FORMATTED')}</span> words
                                </a>
                            </div>
                            ...
                            {this.state.showDownloadProgress ? <div className="chunk-download-progress" /> : ''}
                        </div>
                    ) : null}
                </div>
                <OutsourceContainer
