...
class ProjectContainer extends React.Component {
    ...
    changeUser(value) {
        ...
        if (
            ...
            (this.props.project.get('id_assignee') && idUser != this.props.project.get('id_assignee'))
        ) {
            ...
            menuHtml = (
                <div className="menu">
                    <div className="scrolling menu">
                        ...
                        <a className="item" onClick={this.activateProject.bind(this)}>
                            <i className="icon-drawer unarchive-project icon" /> Resume Project
                        </a>
                    </div>
                </div>
            );
    ...
    getLastAction() {
        let self = this;
        this.props
            .lastActivityFn(this.props.project.get('id'), this.props.project.get('password'))
            .done(function (data) {
    ...
    getJobsList(targetsLangs, jobsList, jobsLength) {
        ...
        orderedJobs.map(function (job, i) {
            ...
            if (tempIdsArray.indexOf(job.get('id')) > -1) {
                isChunk = true;
                index++;
            } else if (orderedJobs.get(i + 1) && orderedJobs.get(i + 1).get('id') === job.get('id')) {
    ...
    createUserDropDown(users) {
        ...
        let members = users.map(function (member, i) {
            ...
            return (
                <div className="item" data-value={user.get('uid')} key={'user' + user.get('uid')}>
                    {userIcon}
        ...
        return (
            ...
            >
                ...
                <div className="ui cancel label" onClick={self.changeUser.bind(self, '-1')}>
    ...
    getDropDownTeams() {
        ...
        if (this.props.teams && this.props.teams.size > 1) {
            let teams = this.props.teams.map(function (team, i) {
                return (
                    <div className="item " data-value={team.get('id')} key={'team-dropdown-item' + team.get('id')}>
                        {team.get('name')}
                    </div>
                );
            });
    ...
    getDueDate() {
        ...
        return <div className="eight wide left aligned column pad-top-0 pad-bottom-0"></div>;
    }
                ...
                <div className="eight wide right aligned column pad-top-0 pad-bottom-0">
                    <div className="activity-log">
                        <a href={activityLogUrl} target="_blank" className="right activity-log" title="Activity log">
                            <i>
                                {' '}
                                <span>Last action: {this.state.lastAction.action + ' on ' + date}</span>
                    ...
                    <div className="activity-log">
                        <a href={activityLogUrl} target="_blank" className="right activity-log" title="Activity log">
                            <i>
                                {' '}
                                <span>
                                    Created on: {this.props.project.get('jobs').first().get('formatted_create_date')}
                                </span>
                            </i>
                        </a>
                    </div>
            ...
            >
                <div className="sixteen wide column">
                    ...
                    <div className="project-body ui grid">
                        <div className="jobs sixteen wide column pad-bottom-0">{jobsList}</div>
                    </div>
