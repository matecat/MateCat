...
const {
    ...
    isAutoGenerated,
    isGitHubWebEdit,
    isCopilotAutofixCommit,
    isPullRequestChecklistCommit,
    stripVS16,
...
describe('isCopilotAutofixCommit', () => {
    it('detects GitHub Copilot Autofix commit', () => {
        assert.equal(
            isCopilotAutofixCommit(
    ...
    it('rejects normal commit', () => {
        assert.equal(isCopilotAutofixCommit('fix: update login flow'), false);
    });
});

describe('isPullRequestChecklistCommit', () => {
    it('detects PR checklist dump commit', () => {
        assert.equal(
            isPullRequestChecklistCommit(
                '## Summary Add Swagger documentation. ## Type - [x] `chore` ## Testing - [x] Manual testing',
            ),
            true,
        );
    });

    it('rejects normal markdown commit', () => {
        assert.equal(isPullRequestChecklistCommit('## Summary only'), false);
    });
});
...
describe('validateCommitMessage – relaxed mode', () => {
    ...
    it('skips Copilot Autofix commit in relaxed mode', () => {
        const r = validateCommitMessage(
            'Potential fix for pull request finding\n\n' +
            'Co-authored-by: Copilot Autofix powered by AI <175728472+Copilot@users.noreply.github.com>',
            relaxed,
    ...
    it('skips PR checklist dump commit in relaxed mode', () => {
        const r = validateCommitMessage(
            '## Summary Add Swagger documentation. ## Type - [x] `chore` ## Testing - [x] Manual testing',
            relaxed,
        );
    ...
    it('does NOT skip Copilot Autofix commit in strict mode', () => {
        const r = validateCommitMessage(
            'Potential fix for pull request finding\n\n' +
            'Co-authored-by: Copilot Autofix powered by AI <175728472+Copilot@users.noreply.github.com>',
        );
    ...
    it('does NOT skip PR checklist dump commit in strict mode', () => {
        const r = validateCommitMessage(
            '## Summary Add Swagger documentation. ## Type - [x] `chore` ## Testing - [x] Manual testing',
        );
        assert.equal(r.valid, false);
