...
/**
 * A release commit, whose whole subject is the version it cuts: `v4.1.3`.
 *
 * These are produced by the release procedure, not written by hand, and they carry no type or
 * description to give them. They used to be created on develop and pushed straight there, where no
 * pull_request workflow ever saw them; a hotfix released from master reaches develop through a
 * back-merge PR instead, and the hygiene gate then rejects a subject nobody is free to reword.
 *
 * Accepts an optional pre-release or build suffix, so `v4.1.3-rc.1` passes too.
 */
function isReleaseCommit(subject) {
    return /^v\d+\.\d+\.\d+(?:[-+][0-9A-Za-z.-]+)?$/.test(subject);
}
...
function validateCommitMessage(message, {requireEmoji = true} = {}) {
    ...
    // Commits only. A pull request titled `v4.1.3` is still a pull request that says nothing.
    if (!requireEmoji && isReleaseCommit(subject)) {
        return {valid: true, errors: [], skipped: true};
    }
