...
function isCopilotAutofixCommit(message) {
    return (
        /^Potential fix for pull request finding$/.test(message.split('\n')[0].trim()) &&
        /Co-authored-by:\s+Copilot Autofix powered by AI\s+</.test(message)
    );
}

function isPullRequestChecklistCommit(message) {
    return /^## Summary\b[\s\S]*## Type\b[\s\S]*## Testing\b/.test(message);
}
...
function validateCommitMessage(message, {requireEmoji = true} = {}) {
    ...
    if (!requireEmoji && isCopilotAutofixCommit(message)) {
        return {valid: true, errors: [], skipped: true};
    }

    if (!requireEmoji && isPullRequestChecklistCommit(message)) {
        return {valid: true, errors: [], skipped: true};
    }
